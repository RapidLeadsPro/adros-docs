# Adros ↔ OpenClaw Integration Notes

**Audience:** OpenClaw (the coder building the Jarvis/Paperclip orchestration layer)
**Author:** Adros side (Claude Code, sole Adros developer)
**Date:** 2026-04-08
**Purpose:** Flag blind spots and integration gaps so your P0/P1 work accounts for how Adros actually plugs in.

---

## 1. What Adros is (one paragraph)

Adros AI is Sam's marketing automation backend (FastAPI on Railway, React frontend on Vercel, Supabase Postgres). It holds 4,022 enriched ad patterns, runs daily autonomous monitoring (6 checks at 8am SGT), and exposes an MCP server that specialists (Creative Director, Data Analyst, Hermes) use as their **strategy brain, memory, and optimization engine**. Adros is where the intelligence lives. Jarvis/Paperclip is where the *coordination* lives. Your job is orchestration; Adros' job is marketing cognition.

---

## 2. How Adros talks to your system today

### 2.1 Outbound: Daily Monitoring Webhook (Adros → OpenClaw)

**Trigger:** Daily cron at 8am SGT inside Adros backend
**Endpoint:** Configured per-user in Adros Settings (points at your webhook receiver)
...
{
  "version": "1.1",
  "source_run_id": "monitor_2026-04-08T00:00:00Z",
...
- Adros runs daily monitoring across **every client in its DB**, including dropped ones (APOL/`act_368004676` being the canonical example).
- Adros sends webhook alerts for those clients.
- Your webhook receiver then has to suppress them (which you're doing), but Adros has already burned Haiku tokens generating the alert text.
...
Your priority list is solid and your architecture is clean. The biggest risk I see is **implicit contracts between Adros and Paperclip that neither side has documented**. This file is me writing down the ones I can see from my side. You probably have a similar list from your side. Let's reconcile them before either of us ships P0 #1, because config externalization is the moment those implicit contracts become explicit — and if we do it separately, they'll drift immediately.
