...
**Audience:** OpenClaw (Jarvis/Paperclip coder)
**Author:** Adros side (Claude Code — verified against repo at `c:\Cursor Projects\Adros\adros`)
**Date:** 2026-04-08
**Adros version:** 0.3.0

> **Rule of reading:** every fact in this doc was pulled from the live repo, not memory. File references point to the authoritative location if you need to dig deeper.
>
> **Companion docs:**
> - `ADROS-INTEGRATION-NOTES-FOR-OPENCLAW.md` — integration blind spots + 5 open decisions
> - `ADROS-ONBOARDING-FLOWS.md` — Journey A (fresh client) vs Journey B (existing ad account), Manus bridge contract
...
- A human chat interface (Sam talks to Jarvis, not Adros — dashboard `app.adros.ai` is for inspection)
...
- Specialists hitting this limit get a clear error — OpenClaw should surface to Sam, not retry
...
Many phases include **`USER CHECKPOINT:` markers** — these are explicit "stop and wait for Sam's approval" gates. Jarvis should honor these as approval pauses, not silently continue.

**Key insight for OpenClaw:** client onboarding is **not a new workflow** — it's a chained sequence of these 9. See `ADROS-ONBOARDING-FLOWS.md` for how Journey A and Journey B chain them.
...
4. **`search_term_analyzer.analyze_search_terms`** → Google, per customer (first 3). Issue if wasted_spend > $20. Critical if > $100. Notify Sam if > $100. Also flags low quality-score keywords (>= 3 issues)
5. **`rsa_asset_monitor.analyze_rsa_assets`** → Google RSAs below "Good" ad strength
6. **`google_conversion_health.check_google_conversion_health`** → health_score 0-100, status `excellent|good|fair|poor`. Notify Sam if status="poor"
...
```python
{
    ...
    "notify_sam": bool,           # True if Sam should be pinged immediately
...
```json
{
  ...
  "issue_count": 12,
  "notify_sam_count": 3,
  "affected_clients": ["ToonieTours", "Pranic Healing"],
  "alert_types": ["creative_fatigue", "search_term_waste", "rsa_health"],
  "issues": [ ... array of issue objects ... ]
}
```

### Webhook delivery details (things that are different from what OpenClaw might assume)
- **POST** with `Content-Type: application/json`, `User-Agent: Adros-Monitor/1.0`
- **Signature header:** `X-Adros-Signature: sha256=<hex>`
- **Signature input:** `json.dumps(payload, sort_keys=True).encode("utf-8")` — YOU MUST sort keys when verifying
- **Timeout:** 15 seconds (`httpx.AsyncClient(timeout=15.0)`)
- **NO automatic retries** in the current code. 5xx / timeout / network error = logged and dropped. If OpenClaw's receiver is down, that day's alerts are lost unless Adros is the one implementing retries. ← **Gap worth fixing on the Adros side.**
...
3. **ALWAYS `auto_select=True`** on `build_creative_prompt` unless Sam explicitly picks a pattern
...
— Claude Code (Adros side, verified against live repo)
