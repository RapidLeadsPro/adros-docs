...
- **External orchestration:** Many customers still pair Adros with Paperclip, OpenClaw, or custom conductors. That is fine: Adros remains the **marketing capability + data plane**; the conductor owns cross-tool sequencing if you want it outside Adros.
...
- `raw_research` (JSONB) — Manus or research-lane findings land here
...
```python
{
    ...
    "recommended_specialist": str,  # runtime actor id: adros_ceo | adros_cmo | adros_cd | adros_deployer
...
```
Same alert type + same client + same headline = same dedupe_key. Your webhook receiver should use this as the authoritative dedupe identifier.
...
- **Adros = hybrid marketing OS:** marketing brain (patterns, memory, creative, monitor) **plus** dashboard orchestration (tasks, timeline, approvals, workspace, files, campaigns). External stacks (Paperclip, OpenClaw, custom) remain optional conductors.
