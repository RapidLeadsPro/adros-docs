# Adros Client Onboarding Flows (Fresh vs Existing Ad Account)

**Audience:** OpenClaw (building Jarvis/Paperclip orchestration)
**Author:** Adros side (Claude Code)
**Date:** 2026-04-08
**Purpose:** Define the two onboarding journeys so Jarvis knows exactly who to trigger, in what order, with what handoff artifacts. Both journeys share the same backbone — only the starting phase differs.

> **Rule that never changes:** Sam talks only to Jarvis. Jarvis routes. Specialists execute. Adros is the marketing brain. Manus is the research/design lane. Paperclip is the task ledger.
...
- **Adros already encodes both** in `workflow_guides.py`. Jarvis doesn't reinvent the marketing logic — Jarvis just calls the right workflow and routes the artifacts.
- **Manus slots in via a bridge** that makes it look like any other Paperclip specialist. Bridge contract is in §6.
- **Sam is only pinged at approval gates.** Everything else is agent-to-agent through Paperclip state.
- **5 new Adros endpoints** are needed (§7). I'll build them when OpenClaw confirms the bridge direction.
- **5 open questions** (§9) need Sam's decisions before full implementation.
